...
function pane.get_position(wibox)
...
function pane.set_position(wibox, position, screen)
...
local function update_all_wiboxes_position()
    for _, wprop in ipairs(wiboxes) do
        pane.set_position(wprop.wibox, wprop.position, wprop.screen)
    end
end
...
function pane.attach(wibox, position, screen)
...
function pane.align(wibox, align, screen)
    local position = pane.get_position(wibox)
...
function pane.stretch(wibox, screen)
    if screen then
        local position = pane.get_position(wibox)
        local area = capi.screen[screen].workarea
...
function pane.new(arg)
    ...
    pane.attach(w, position, screen)
    if has_to_stretch then
        pane.stretch(w, screen)
    else
        pane.align(w, arg.align, screen)
    end

    pane.set_position(w, position, screen)
...
function pane.mt:__call(...)
    return pane.new(...)
end

return setmetatable(pane, pane.mt)
